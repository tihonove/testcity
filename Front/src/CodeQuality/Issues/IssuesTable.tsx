...
import { SEVERITY_VALUES } from '../Overview/utils';
  ...
  return (
    <div>
      <table>
        <thead>
          <tr>
            <th onClick={() => setSort((prev) => changeSort(prev, 'severity'))}>Severity</th>
            <th onClick={() => setSort((prev) => changeSort(prev, 'category'))}>Category</th>
            <th onClick={() => setSort((prev) => changeSort(prev, 'id'))}>ID</th>
            <th onClick={() => setSort((prev) => changeSort(prev, 'path'))}>Description</th>
          </tr>
        </thead>
        <tbody>
          {paged.map((x) => (
            <tr>
              ...
              <td>
                {x.description}
                <br />
                {x.location.path}:{x.location.lines?.begin ?? x.location.positions?.begin?.line}
                <br /> {x.content?.body}
              </td>
            </tr>
          ))}
      ...
      <Paging
        ...
        pagesCount={Math.ceil(report.length / PAGE_SIZE)}
      />
    </div>
  );
}
...
function sortIssues(issues: Issue[], sort: SortState | null): Issue[] {
  ...
  return issues.toSorted((a, b) => {
    ...
    if (sort.field === 'path') {
      const p1 = `${a.location.path}:${a.location.lines?.begin ?? a.location.positions?.begin?.line}`;
      const p2 = `${b.location.path}:${b.location.lines?.begin ?? b.location.positions?.begin?.line}`;
